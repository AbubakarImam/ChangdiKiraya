...
public class GetUserBookingsQueryHandler : IRequestHandler<GetUserBookingsQuery, PagedResult<BookingDto>>
{
    ...
    public async Task<PagedResult<BookingDto>> Handle(GetUserBookingsQuery request, CancellationToken cancellationToken)
    {
        var pagedBookings = await _bookingRepository.GetUserBookingsAsync(request.UserId, request.PageNumber, request.PageSize, cancellationToken);
